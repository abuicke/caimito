...
private class Thrower(private val throwable: (String, Throwable?) -> Exception) {

    fun throwException(message: String, cause: Throwable?): Nothing {
...
private fun <T> error(
    ...
    return if (AppConfiguration.shouldCrashOnError()) {
        thrower.throwException(message, cause)
